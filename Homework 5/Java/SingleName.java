/*
 * Objective: Creates SingleName object and formats it to "Aaaa" style
 * 						inherits from AlphaString and overrides Object's toString() method.
 * 						Overrides toString, clone and equals.
 */

public class SingleName extends AlphaString implements Cloneable {

	private AlphaString s;

	public SingleName(AlphaString s) {
		this.s = s;
		set(s);
	}

	public SingleName() {
	}

	// copy constructor
	SingleName(SingleName as) {
		s = as.s;
	}

	public String get() {
		return s.get();
	}

	public void set(AlphaString s) {

		char[] sa = s.get().toCharArray();

		for (int i = 0; i < sa.length; i++) {
			sa[i] = Character.toLowerCase(sa[i]);
		}
		sa[0] = Character.toUpperCase(sa[0]);
		s.set(String.valueOf(sa));

	}

	@Override
	public String toString() {
		return s.get();
	}

	@Override
	public SingleName clone() {
		SingleName obj;
		obj = (SingleName) super.clone();
		return obj;
	}

	@Override
	public boolean equals(Object obj) {
		if (obj == null) {
			return false;
		}
		if (!SingleName.class.isAssignableFrom(obj.getClass())) {
			return false;
		}
		final SingleName other = (SingleName) obj;

		if ( !this.s.equals(other.s)) {
			return false;
		}

		return true;
	}
}
